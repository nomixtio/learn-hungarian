...
- **Pinned versions, do not "fix"**: `alchemy@2.0.0-beta.77` requires `effect@4.0.0-rc.112` (lowercase `Config.*` API — newer RCs renamed to `Config.String` and break the Alchemy CLI itself). React must stay `^19.2.7` (dedupes Alchemy's nested copy; duplicates crash its CLI). `@testing-library/dom` is an explicit dep because legacy-peer-deps skips peers.
